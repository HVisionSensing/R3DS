...
public:
    ...
    template <class ErrorFunction>
    static ProblemVector gradientDescent(const QVector<QVector3D> &meshFrom, const QVector<QVector3D> &meshTo, const ErrorFunction &func, UpdateFunctor *update = NULL);
};
...
template<class ErrorFunction>
ProblemVector GradientDescent::gradientDescent(const QVector<QVector3D> &meshFrom, const QVector<QVector3D> &meshTo, const ErrorFunction &errorFunc, UpdateFunctor *update)
{
    Settings settings(500000, 0.0001, 1.9, 0.001);
    float function = 0;
    ...
    for (int index = 0; index < settings.stepsCount; index++){
        ...
        function = errorFunc(meshFrom, meshTo, x.angle, x.shiftX, x.shiftY, x.shiftZ, x.axis);
        entireGradient = GradientDescent::gradientAnalytical(meshFrom, meshTo, x, errorFunc);
        entireGradientSecond = GradientDescent::gradientNumerical(meshFrom, meshTo, x, errorFunc);
